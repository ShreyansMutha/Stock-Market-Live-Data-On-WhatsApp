...
class Nse:
    ...
    def __init__(self):
        ...
        self.__symbol_files = {i.name: f"{self.dir['symbol_list']}{i.name}.pkl" for i in IndexSymbol}
    ...
    def __startup(self):
        ...
        self.__urls = self.__read_object(f'{os.path.split(__file__)[0]}/config/config', Format.pkl)
    ...
    def get_quote(self,
        ...
        if symbol is not None:
            ...
            if segment == 'EQ':
                ...
                self.strike_list = [float(s) for s in self.strike_list]
                # print(self.strike_list)
                ...
                # filter data for expiry and strike price
                data = [i for i in data if
    ...
    def bhavcopy(self, req_date: dt.date = None,
        ...
        req_date = self.trading_days()[-1].date() if req_date is None else req_date
    ...
    def option_chain(self, symbol: str = 'NIFTY', req_date: dt.date = None) -> dict:
        ...
        # assume download is required
        download_req = True

        # and eod file exists
        filename = f"{dir}{dt.date.today()}_eod.pkl"

        # first of all check if a file with todays eod exists
        if os.path.exists(filename):
            download_req = False

        # if reqdate not given
            ...
            # if timestamp is today and market is not closed
            # latest option chain is requested
            # download and save
            if timestamp.date() == dt.date.today() and timestamp.time() <= dt.time(15, 30):
                # filename as per timestamp
                filename = f"{dir}{dt.date.today()}_{dt.datetime.now().strftime('%H%M%S')}.pkl"
                download_req = True

            # if timestamp is today and market is closed
            elif timestamp.date() == dt.date.today():
                # filename as eod
                filename = f"{dir}{dt.date.today()}_eod.pkl"

                # read if this file exists
                download_req = False if os.path.exists(filename) else True

            # if timestamp is not for today
            else:
                # get last trading day
                prev_trading_day = self.trading_days()[-1].date()
        ...
        # if reqdate is provided
            ...
            if req_date == dt.date.today():
                ...
                prev_trading_day = self.trading_days()[-1]
                if req_date >= prev_trading_day:
                    # read if exists else download
                    filename = filename = f"{dir}{prev_trading_day}_eod.pkl"
                    download_req = False if os.path.exists(filename) else True
                else:
                    # read if eod file exists
                    filename = filename = f"{dir}{req_date}_eod.pkl"
                    download_req = False
    ...
    def __symbol_list(self, index: IndexSymbol):
        ...
        if index == IndexSymbol.All:
            data = list(self.bhavcopy(req_date=dt.date(2020, 10, 8)).reset_index().SYMBOL)
